  ...
  const fetchData = async (currentPage = 1) => {
    try {
      ...
      const config = {
        ...
        url: `${process.env.REACT_APP_API_PRODUCT}product/inventories?limit=${limit}&offset=${currentPage}&status=${5}`,
        method: 'POST',
        data: body,
      };
  ...
  return (
    <div className="container mx-auto w-full mt-16 lg:mt-0">
      ...
      <div className="flex flex-wrap w-full  mt-2 items-center gap-2">
        ...
        {selected.length !== 0 ? (
          <div>
            <button className={`bg-blue-500 hover:bg-grey-200  font-normal py-2 px-[28px] rounded-lg inline-flex items-center gap-3`}>
              <span className={`text-white`}>
                <AiFillPrinter />
              </span>
              <span className={`text-white`}>
                <Link to="" state={{ dataku: selected }}>
                  Cetak Return
                </Link>
                {/*Cetak Return */}
              </span>
          ...
          <div>
            <button className={`bg-slate-500 hover:bg-grey-200 font-normal py-2 px-[28px] rounded-lg inline-flex items-center gap-3`} disabled>
              <span className={`text-white`}>
                <AiFillPrinter />
              </span>
              <span className={`text-white`}>Cetak Return</span>
            </button>
          </div>
        ) : null}
      </div>
          ...
          <div className="mt-3 flex items-center shadow-md rounded-xl p-3 border-2 justify-between">
            <div className="flex items-center">
              ...
              <div className="flex flex-col text-sm lg:text-base gap-1 text-clip overflow-hidden">
                ...
                <p className="text-xs">Principle : {item.product_principal_name}</p>
                <p className="text-xs">Jumlah : {item.stock}</p>
                <p className="font-bold">Rp. {item.sell_price}</p>
              </div>
            </div>

            <div className="flex flex-col items-center gap-2">
              <button className="shadow-md rounded-xl bg-blue-200 px-3 rounded-xl hidden lg:block font-medium text-sm py-1 hover:bg-green-600 hover:text-white">Cetak Return</button>
              <button className="shadow-md rounded-xl bg-blue-200 px-3 rounded-xl block lg:hidden font-medium text-sm py-1 hover:bg-green-600 hover:text-white">
                <AiFillPrinter />
              </button>
              <p className="text-red-500 text-xs text-center">Tanggal Expired : {item.expired_date}</p>
            </div>
          </div>
        );
      })}
