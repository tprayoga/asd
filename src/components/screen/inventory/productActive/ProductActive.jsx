  ...
  const fetchData = async (currentPage = 1) => {
    try {
      ...
      const config = {
        ...
        url: `${process.env.REACT_APP_API_PRODUCT}product/inventories?limit=${limit}&offset=${currentPage}&name=${search}&status=${1}`,
        method: "POST",
        data: body,
      };
      ...
      {products && products.length > 0 ? (
        <>
          ...
          {products
            ...
            //     setSearch('');
            //     return fil;
            //   } else if (fil.product_name.toLowerCase().includes(search.toLowerCase())) {
            //     return fil;
            //   }
